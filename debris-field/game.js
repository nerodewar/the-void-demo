(() => {
  ...
  class DebrisGame {
      ...
      for (
        ...
        if (
          ...
          if (this.shieldReady) {
            ...
            showToast(
              `${this.engineSide} ENGINE OFFLINE`,
              true,
              2100
            );
          } else {
    ...
    updateHud() {
      ...
      ui.kills.textContent =
        String(this.kills);

      ui.combatScore.textContent =
        this.killScore.toLocaleString();
      ...
      ui.engineBars.classList.toggle(
        'damaged',
        this.engineOffline
      );
    ...
    finish() {
      ...
      ui.stars.setAttribute(
        'aria-label',
        `${stars} star rating`
      );

      ui.resultNarrative.textContent =
        narrative;

      ui.resultDistance.textContent =
        `${this.distance.toFixed(1)} LY`;

      ui.resultCombat.textContent =
        this.killScore.toLocaleString();

      ui.resultOverall.textContent =
  ...
  async function initialise() {
    ...
    try {
      ...
      ui.loadingCopy.textContent =
        'Unable to initialise flight assets. Reload the page and try again.';

      ui.loadingCopy.style.color =
        '#ff7b83';
    }
  }
